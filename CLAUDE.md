...
**auri** is a library of agent-facing component catalogs for the [A2UI](https://a2ui.org) protocol, built on [svelte-a2ui](https://github.com/ChaliceForAuri/a2ui-svelte) — the maintainer's own Svelte 5 renderer. Where the renderer is infrastructure (spec-faithful, boring, trustworthy), auri is the product surface: ready-made, beautiful, LLM-emittable component vocabularies. The shadcn move, aimed at the catalog layer. **The space is not empty** (verified 2026-08-21): vercel-labs/json-render has ~16k stars and ships a Svelte target — but its 36 components are a shadcn _primitive mirror_ with no Chart, Stat, Sparkline, Timeline or approval flow, and the A2UI basic catalog's 18 components carry **no intent/severity/status concept at all**. The wedge is the opinionated semantic vocabulary, not the existence of a catalog.
...
2. **The prompt-pack** — `contract/prompt.md` + `contract/examples/*.jsonl`: the system-prompt text and few-shot fixtures that teach an agent the vocabulary. Half the product. **Not unique — do not claim it is**: A2UI's own SDK generates system prompts (`A2uiSchemaManager.generate_system_prompt`), as do json-render and OpenUI. What _is_ ours is that every rule in the pack was earned from an observed model failure (the envelope-brace trap, component batching, the action-shape trap) rather than derived mechanically from the schema.
...
9. **No catalog ships an enum that names someone's business.** UI concepts get closed enums (`line|bar|area`, the intent scale); domain vocabulary travels as _data_ — a free-string `kind`, a general `metrics: [{label, value, unit, intent}]` array. A closed enum is an exclusion: `signalType: churn_risk|expansion|…` is unusable by a security product, and one customer's entity model in the schema makes the catalog theirs instead of everyone's.
...
**M4 (launch) done; post-launch shipped (2026-08-19):** brand settled (flame-only mark + Geist Mono logotype — see memory `brand-identity`, and the large-scale glow gotcha there); OG card refreshed; `/playground` (six unedited GPT-5.6 harness transcripts in `src/lib/playground-data.json`, replayed into live Surfaces — doubles as a regression corpus); `/foundations/shadcn` (two-lane stance + token bridge); **the catalog composer (M5)** — `@aurilabs/core/compose` (pure, zero-dep, tested against real ops artifacts), harness `--pack`/`--contract` flags (`validate-stream` exposes `createValidator`; a composed contract fails vocabulary escapes), and the live `/composer` page. Ecosystem: listing PR a2ui-project/a2ui#2290 + Discussion #2291 are Hugo's and live — do not open new ones (memory `a2ui-ecosystem-submission`). **The forms catalog: M1 gate PASSED and M3 implementation COMPLETE (2026-08-19).** `packages/forms` — contract (10 components, gate log in `contract/README.md`: GPT-5.6 6/6 final pack after the component-batching rule, Claude Fable 3/3 + Sonnet 3/3 fresh sessions) and live Svelte (`src/lib/`, mirrors ops patterns; 13 browser tests + 5 contract tests; package+publint green). Two catalog-layer semantics to know: **fields compute their own validation** (the renderer resolves check args verbatim — value injection is the catalog's job, see `src/lib/checks.ts`), and **SubmitBar computes surface-wide validity** from `client.state` + `surfaceValid()`. Package is `private: true`, version 0.0.0 — flipping public needs Hugo's manual first npm publish + Trusted Publishing config + release.yml addition + lockstep join. Remaining forms work: docs integration (component pages, sheet, playground scenarios), composer second source (compose.js pack merger is single-source v1), then publish.
