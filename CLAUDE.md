...
M0 (scaffold) done. **Current milestone: M1 — contract-first design** of all 12 ops components on paper (`catalog.json` + `prompt.md` + one fixture each), then the LLM-emission gate. Progress 2026-08-17: the first six — `Stat`, `Badge`, `Callout`, `DataTable`, `ApprovalCard`, `Chart` — are drafted, green under ajv contract tests (`npm test`), and **have passed the emission gate across two model families** (Claude Fable + Sonnet, GPT-5.6; log in `packages/ops/contract/README.md`). The eval harness is `npm run eval` in `packages/ops` (providers activate on env keys; `mock:pass` tests the pipeline offline). Next: the remaining six — `Timeline` first, since it's the only component needing the collection-template mechanic (`{path, componentId}` child templates) the prompt-pack doesn't teach yet, then `Sparkline`, `Progress`, `KeyValue`, `CodeBlock`, `ConfirmButton`. No Svelte until M1 passes.
